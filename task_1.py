...
if __name__ == "__main__":
    ...
    terminals = [("Terminal 1", 0), ("Terminal 2", 1)]
    shops = [
        ("Shop 1", 6),
        ("Shop 2", 7),
        ("Shop 3", 8),
    ...
    for terminal in terminals:
        for shop in shops:
            print(
                f"| {terminal[0].ljust(15)} | {shop[0].ljust(15)} | {str(edmonds_karp(capacity_matrix, terminal[1], shop[1])).ljust(5)} |"
            )

total_flow = []

# Розрахуємо сумарну кількість потоку для кожного термінала
for terminal in terminals:
    flow = 0
    for shop in shops:
        flow += edmonds_karp(capacity_matrix, terminal[1], shop[1])
    total_flow.append((terminal[0], flow))

print("\nЗагальний потік:")
for name, amount in total_flow:
    print(f"{name}: {amount}")
